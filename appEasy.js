...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			let ordenadas = movies
			ordenadas.sort(function(a,b){
			    return((a.title<b.title)?-1:((a.title>b.title)?1:0))
		      })
			res.write(`\n➢ Listado de peliculas:\n\n`)
		    ordenadas.forEach(function(movie){
		...
		case '/en-cartelera':
			...
			let pelis=movies.forEach((elemento)=>{
				res.write(
		...
		res.write(`➢ Total de películas​: ${votadas.length} \n\n`)
		let promedio=0
		...
		case '/sucursales':
			...
			let salas=theaters.forEach((elemento)=>{
		...
		case '/preguntas-frecuentes':
			...
			let question=faqs.forEach((elemento)=>{
				res.write(`\n ${elemento.faq_title}
		     \n• ${elemento.faq_answer}\n\n`)
			})			
